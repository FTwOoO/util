...
import (
	...
	"gitlab.livedev.shika2019.com/soccer-go/biz-bot/util/redisqueue"
	"testing"
	"time"
)

func TestQueueTasks(t *testing.T) {
	q := redisqueue.New("basic_queue", &cfg.RedisConfig{Host: "127.0.0.1", Port: 6379})
	err := q.FlushQueue()
	if err != nil {
		t.Error(err)
		t.FailNow()
...
func TestQueueTaskScheduling(t *testing.T) {
	q := redisqueue.New("scheduled_queue", &cfg.RedisConfig{Host: "127.0.0.1", Port: 6379})
